...
function main_menu {
    while true; do
		...
		clear
        logo 
printColor blue "● Maintenance Menu Side Protocol
│
│ ┌───┬──────────────────────────────────────┐
...
read -p "Make your choice and enter the item number ► " choice
		...
		1)	
            ...
            echo ""
            sided status 2>&1 | jq -r '.ValidatorInfo // .validator_info' && sleep 2
            echo ""	
			;;
		2)	
			echo ""
		...
		3)	
            ...
            echo ""
			echo $(sided tendermint show-node-id)'@'$(curl -s ifconfig.me)':'$(cat ~/.side/config/config.toml | sed -n '/Address to listen for incoming connection/{n;p;}' | sed 's/.*://; s/".*//') && sleep 2
            echo ""	
			;;
		4)	
			echo ""
		...
		5)	
            ...
            echo ""
            sed -i -e "s/^minimum-gas-prices *=.*/minimum-gas-prices = \"0.005uside\"/" ~/.side/config/app.toml && sleep 2
            echo ""
			;;
		6)	
			echo ""
            ...
            echo ""
            sided tendermint unsafe-reset-all --keep-addr-book --home ~/.side && sleep 2
            echo ""
			;;
		0)
			echo ""
