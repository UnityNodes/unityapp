...
read -p "Зробіть ваш вибір, та введіть номер пункту ► " choice
		...
		5)	
            ...
            echo ""
            cascadiad q bank balances $(cascadiad keys show wallet -a) && sleep 2
            echo ""
			;;
		6)	
  			echo ""
		...
		7)	
            ...
            echo ""
            cascadiad tx slashing unjail --from wallet --chain-id cascadia_11029-1 --gas-prices 0.1aCC --gas-adjustment 1.5 --gas auto -y && sleep 2
            echo ""
			;;
		8)
            echo ""
		...
		10)
            ...
            echo ""
            cascadiad q staking validator $(cascadiad keys show wallet --bech val -a); cat $HOME/.cascadiad/config/priv_validator_key.json && sleep 2
            echo ""
			;;	
		0)
			echo ""
