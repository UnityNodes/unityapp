...
function main_menu {
    while true; do
		clear
        logo 
printGreen "● Сервісні команди ⚙️ Dymension: 
│
│ ┌───┬──────────────────────────────────────┐
├─┤ 1 │ Старт ноди                           │
│ ├───┼──────────────────────────────────────┤
...
read -p "Зробіть ваш вибір, та введіть номер пункту ► " choice
		...
		4)	
			echo ""
            printGreen "↓ Журнал логів Dymension ↓ Натисніть CTRL+C щоб вийти ↓" && sleep 1
            echo ""
		...
		7)	
            ...
            echo ""
            dymd version; dymd status | jq .NodeInfo.version| tr -d '"' && sleep 2
            echo "" 
			;;
		0)
			echo ""
